...
function endsWithStart($haystack) {
    return substr($haystack, -strlen('start')) === 'start';
}
...
class helper_plugin_include extends DokuWiki_Plugin { // DokuWiki_Helper_Plugin
    ...
    function helper_plugin_include() {
		...
		$this->defaults['start']    = 1;
		$this->defaults['exclude']  = '';
    }
        ...
        case 'namespace':
            ...
            if (is_array($pagearrays)) {
                foreach ($pagearrays as $pagearray) {
					...
					if ($flags['start'] == '0' && endsWithStart($pagearray['id']))
						$includePage = false;
					
					if ($flags['exclude'] != '' && $flags['exclude'] == $pagearray['id'])
					$includePage = false;
